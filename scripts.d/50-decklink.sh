...
ffbuild_dockerstage() {
    ...
    to_df '    # widl (wine 10) does not accept Microsoft MIDL\\047s C++ `bool` keyword. \\'
    ...
    to_df '    # DeckLinkAPI.idl uses C #include (not IDL import) to pull in every \\'
    to_df '    # versioned .idl, so widl produces one DeckLinkAPI.h with all interfaces. \\'
    to_df '    # The versioned .idl files are NOT standalone-compilable (they expect to \\'
    to_df '    # be #included into DeckLinkAPI.idl). Generate stub headers that redirect \\'
    to_df '    # to DeckLinkAPI.h to satisfy ffmpeg\\047s versioned #include directives. \\'
    to_df '    "$WIDL" -h -H DeckLinkAPI.h DeckLinkAPI.idl && \\'
    to_df '    "$WIDL" -u -U DeckLinkAPI_i.c DeckLinkAPI.idl && \\'
    to_df '    for idl in DeckLinkAPI_*.idl DeckLinkAPIConfiguration.idl DeckLinkAPIDeckControl.idl \\'
    to_df '              DeckLinkAPIDiscovery.idl DeckLinkAPIModes.idl DeckLinkAPIStreaming.idl \\'
    to_df '              DeckLinkAPIStreaming_v10_8.idl DeckLinkAPITypes.idl; do \\'
    to_df '        [ -f "$idl" ] && echo "#include \\"DeckLinkAPI.h\\"" > "${idl%%.idl}.h"; \\'
    to_df '    done && \\'

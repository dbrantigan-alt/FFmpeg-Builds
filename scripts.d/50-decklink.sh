...
ffbuild_dockerstage() {
    ...
    to_df '    # Each versioned .idl is designed to be compiled alongside DeckLinkAPI.idl which \\'
    to_df '    # imports unknwn.idl (provides IUnknown). For standalone widl compilation, prepend \\'
    to_df '    # the import to every .idl that doesn\\047t already have it. \\'
    to_df '    for idl in *.idl; do \\'
    to_df '        grep -q "import \\"unknwn.idl\\"" "$idl" || \\'
    to_df '            sed -i "1i import \\"unknwn.idl\\";" "$idl"; \\'
    to_df '    done && \\'
